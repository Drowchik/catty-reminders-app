...
Authentication supports two modes via `auth_mode` in [`config.json`](config.json):

### Local mode (`auth_mode: "local"`)

Users and passwords are declared in `config.json`.
Optional `user_roles` map usernames to `admin` (default) or `viewer` (read-only).

Example users:

| Username | Password | Role |
|----------|----------|------|
| heisenberg | P@ssw0rd | admin (full access) |
| tester | foobar123 | viewer (read-only) |

### Active Directory mode (`auth_mode: "ldap"`)

Login uses LDAP bind on port 636 (LDAPS). Roles come from AD group membership:

- `ldap.admin_group` → can create, edit, and delete reminders; can register users via API
- `ldap.viewer_group` → read-only access

Set service account credentials via environment variables (recommended):

```bash
export LDAP_BIND_USER="admin@todolist.lan"
...
Create matching security groups in AD and update `admin_group` / `viewer_group` DNs.

### Registering users in AD (admins only)

When running in LDAP mode, admins may call:

```http
POST /api/admin/users
Content-Type: application/json

{"email": "newuser@todolist.lan", "password": "SecurePass123!"}
```

This creates the user under `ldap.staff_ou` and activates the account.
