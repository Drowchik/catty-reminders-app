...
from app.utils.auth import get_storage_for_api, register_ad_user, require_admin_api
...
class RegisterAdUser(BaseModel):
  email: str
  password: str


class RegisterAdUserResult(BaseModel):
  success: bool
  message: str
...
# --------------------------------------------------------------------------------
# Active Directory user management (admins only)
# --------------------------------------------------------------------------------

@router.post(
  path="/admin/users",
  summary="Register a new Active Directory user",
  response_model=RegisterAdUserResult
)
async def post_admin_register_user(
  body: RegisterAdUser,
  _admin=Depends(require_admin_api)
) -> RegisterAdUserResult:
  if auth_mode != "ldap":
    return RegisterAdUserResult(success=False, message="Available only when auth_mode is ldap")

  success, message = register_ad_user(body.email, body.password)
  return RegisterAdUserResult(success=success, message=message)
